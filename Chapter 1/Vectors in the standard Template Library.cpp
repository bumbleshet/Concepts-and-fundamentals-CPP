#include "stdafx.h"
/*simpliest STL container is vector-contiguous blocks of memory just like an array. they can be
randomly accessed so that the access time of any element of the vector is constant. Storage is managed automatically so that on an attempt to insert
an element into a full vector, a larger memory block is allocated for the vector, vector elements are copied to the new block, and the ild block is released. A vector is thus a flexible aray;
that is, an array whose size can be dynamically changed.
*/
...
//T&=reference type
//front() return a value pero reference japon ko ah
...
int main() {
	...
	replace_if(v5.begin(), v5.end(), bind2nd(less<int>(),4), 7);//bind2nd generic function behaves like converting 2 arg to 1 arg by binding second parameter(op takes a as the second argument)
